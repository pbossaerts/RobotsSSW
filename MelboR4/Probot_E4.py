...
Nplayer = 9 # Careful: NINE players!!
...
for i in range(Nplayer):
    ...
    for j in range(len(robotlogbooks[i])):
        if not (robotlogbooks[i][j][0] == '' or robotlogbooks[i][j][0] == '===========' or robotlogbooks[i][j][0] == '============'): # This is different from experiment 1
...
timeadjustment = '11:00:00'  # Unlike previous experiments, this one was run after switch to Northh Winter
...
# reference value, number of orders, participant (added 5/2023)
...
market = 191 #  market
begin_session = 2661  # Round 1
end_session = 2675  # Round 15
min_time = 082151.077  # min createDate (hours:min:sec.ms without ":")
max_time = 092716.056  # max lastModifiedDate
                            ...
                            if market_book[j][10] != 'CANCEL':
                                if market_book[j][9].isnumeric():
                                    if int(market_book[j][6]) > int(market_book[j][9]) > 0:
                                        ...
                                        temp += 1  # count trades
                                        tempA.append(market_book[j][6])  # Order ID
                                        tempA.append(market_book[j][8])  # S(upplier) ID
                                        tempA.append(market_book[j][9])  # C(onsumer) ID
...
for i in range(15):
    ...
    for j in range(len(active_book)):
        for k in range(len(active_book[j])):
            ...
            c = active_book[j][k][7][
                0:8] == 'An order'  # This identifies records in robot log that confirm an order submissio
            if a and b and c:
                temp += 1
    ...
    for j in range(len(active_book)):
        for k in range(len(active_book[j])):
            if active_book[j][k][7][0:15] == 'An order was ac':
                # This entry in robot log states the order was accepted and it will contain the Flex-E-Markets order number
                a = int(active_book[j][k][2]) >= start_time
                b = int(active_book[j][k][2]) <= end_time
                if a and b:
                    period_orders.append(active_book[j][k][7].split(":")[2])
    robot_orderID_each_period.append(period_orders)
...
for session in range(begin_session, end_session + 1):
    ...
    for j in range(len(trade_orderID_each_period[i])):
        ...
        vlag = 0
        for k in range(len(robot_orderID_each_period[i])):
            a = robot_orderID_each_period[i][k] == trade_orderID_each_period[i][j][1]  # Is SID = robot order?
            b = robot_orderID_each_period[i][k] == trade_orderID_each_period[i][j][2]  # Is CID = robot order?
            c = vlag == 0
            if (a or b) and c:
                temp += 1
                vlag = 1
...
for session in range(begin_session, end_session + 1):
    ...
    for j in range(len(trade_orderID_each_period[i])):
        ...
        b = 0
        for k in range(len(robot_orderID_each_period[i])):
            if robot_orderID_each_period[i][k] == trade_orderID_each_period[i][j][1]:
                a = 1  # Is SID = robot order?
            if robot_orderID_each_period[i][k] == trade_orderID_each_period[i][j][2]:
                b = 1  # Is CID = robot order?
        if (a and b):
            temp += 1
