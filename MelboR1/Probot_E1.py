...
for i in range(Nplayer):
    ...
    for j in range(len(robotlogbooks[i])):
        if not (robotlogbooks[i][j][3] == '42'or robotlogbooks[i][j][3] == ''):
...
# reference value (fv), etc. May 2023: PB added identity of user at end of records
...
# Output:
# robot_info
# This is a list is 3D: [1] player, [2] robot, [3] start time, stop time, duration, type, side, reference value, number of orders, participant (added 5/2023)
...
df = pd.DataFrame(total_robot_period)
df.columns = ['number']
df.to_csv('robots_per_period.csv')
...
for i in range(15):
    ...
    for j in range(len(active_book)):
        for k in range(len(active_book[j])):
            ...
            c = active_book[j][k][7][0:8] == 'An order'  # This identifies records in robot log that confirm an order submissio
            if a and b and c:
                temp += 1
